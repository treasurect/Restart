...
class EditPayPwdView : AppCompatEditText {
    ...
    private var mRectHeight: Float = 0f//矩形的边框

    private var mTextType: Int = 0//输入框类型

    private var mSpacing: Float = ScreenUtils.dp2px(ContextHolder.app(),16f).toFloat()//间距
        ...
        if (len!! < mLength) {
            if (mCursorDirection) {
                ...
                canvas?.drawLine(
                    ...
                    mRectHeight.toFloat() - mCursorMarginBottom,//减去距离底部的边距
                    endX - mCursorSpacing,
                    mRectHeight.toFloat() - mCursorMarginBottom,
                    mCursorPaint!!
                )
            }
        }
    ...
    private fun canvasInputBox(canvas: Canvas?) {
        ...
        for (a in 0 until mLength) {
            ...
            val bottom = mRectHeight.toFloat()
...
//            height = ScreenUtils.dp2px(context,42f)
//        }
